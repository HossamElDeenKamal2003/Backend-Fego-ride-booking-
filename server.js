...
app.post('/create-payment-link', async (req, res) => {
  ...
  const options = {
    ...
    headers: {
      'Authorization': '46UJMFaY.zc2mwHxLf0QaBfzQr6Rt3LzuGivevs0h',
    },
    data: paymentData
  };
...
app.get('/res', async(req, res)=>{
  axios({
    method: "get",
    url:
      "https://community.xpay.app/api/v1/communities/639/transactions/cfab6941-f80e-4edc-af91-c8cd75eaebbd",
    headers: {
...
const Subtractration = require('./model/subscription'); // Ensure the model name matches
...
// Function to update stats for all drivers
const updateStats = async () => {
  try {
    const drivers = await Subtractration.find(); // Fetch all drivers
    for (let i = 0; i < drivers.length; i++) {
      await decrementSubtractrationDays(drivers[i]._id); // Use drivers[i]._id to access each driver's ID
    }
  } catch (error) {
...
// Optionally, run updateStats every 1 second (1000 ms)
